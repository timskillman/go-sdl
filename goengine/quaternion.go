...
func (q *Quat) GenGoSet(path string) string {
	aa := q.ToAxisAngle()
	return fmt.Sprintf("%s.SetFromAxisAngle(mat32.V3(%g, %g, %g), %g)", path, aa.X, aa.Y, aa.Z, aa.W)
}
...
func (q *Quat) SetFromRotationMatrix(m *Mat4s) {
	...
	if trace > 0 {
		...
		s = 2.0 * math32.Sqrt(1.0+m11-m22-m33)
		q.W = (m32 - m23) / s
		q.X = 0.25 * s
		q.Y = (m12 + m21) / s
		q.Z = (m13 + m31) / s
	} else if m22 > m33 {
		s = 2.0 * math32.Sqrt(1.0+m22-m11-m33)
		q.W = (m13 - m31) / s
		q.X = (m12 + m21) / s
		q.Y = 0.25 * s
		q.Z = (m23 + m32) / s
	} else {
		s = 2.0 * math32.Sqrt(1.0+m33-m11-m22)
		q.W = (m21 - m12) / s
		q.X = (m13 + m31) / s
		q.Y = (m23 + m32) / s
		q.Z = 0.25 * s
